  ...
  try {
    ...
    if (rol === "usuario" && persona_id) {
      const checkPersona = await pool.query(
        "SELECT * FROM usuarios WHERE persona_id = $1",
        [persona_id]
      );
    ...
    if (err.code === "23503") {
      return res
        .status(400)
        .json({ error: "La persona seleccionada no existe o no es válida." });
    }
...
const loginUsuario = async (req, res) => {
  try {
    ...
    const userQuery =
      "SELECT id, nombre_usuario, contrasena_hash, rol, persona_id FROM usuarios WHERE LOWER(nombre_usuario) = LOWER($1)";
    ...
    if (usuario.rol === "usuario" && usuario.persona_id) {
      ...
      const oficinaResult = await pool.query(oficinaQuery, [
        usuario.persona_id,
      ]);
...
const solicitarCodigoReseteo = async (req, res) => {
  ...
  try {
    const personaQuery = await pool.query(
      "SELECT correo FROM persona WHERE id = $1",
      [personaId]
    );
...
const verificarCodigoReseteo = async (req, res) => {
  ...
  try {
    ...
    if (!user || !user.reset_token) {
      return res
        .status(400)
        .json({ error: "Código inválido o no solicitado." });
    }
...
const actualizarPasswordConCodigo = async (req, res) => {
  ...
  try {
    ...
    if (!user || !user.reset_token) {
      return res
        .status(400)
        .json({ error: "Código inválido, expirado o ya utilizado." });
    }
    if (new Date(user.reset_token_expires) < new Date()) {
