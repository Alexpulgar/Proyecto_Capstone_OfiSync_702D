...
describe("Prueba de Integración para el componente Agregar", () => {
  ...
  test("debería llamar al servicio agregarEdificioApi al llenar y enviar el formulario de edificio", async () => {
    edificioService.agregarEdificioApi.mockResolvedValue({
    ...
    fireEvent.change(screen.getByLabelText("Nombre Edificio"), {
      target: { value: "Edificio de Prueba" },
    });
    fireEvent.change(screen.getByLabelText("Pisos Totales"), {
      target: { value: "10" },
    });
    fireEvent.change(screen.getByLabelText("Área bruta por piso (m²)"), {
      target: { value: "500" },
    });
    fireEvent.change(screen.getByLabelText("Área común (%)"), {
      target: { value: "20" },
    });

    const botonesAgregar = screen.getAllByRole("button", {
      name: /agregar edificio/i,
    });
    ...
    await waitFor(() => {
      expect(global.alert).toHaveBeenCalledWith(
        "Edificio Edificio de Prueba agregado correctamente"
      );
    });
  });
});
