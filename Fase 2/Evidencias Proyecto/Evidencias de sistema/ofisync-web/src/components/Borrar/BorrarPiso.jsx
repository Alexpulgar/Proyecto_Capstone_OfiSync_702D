...
import { getEdificios } from "../../../services/edificioService"; // Para el <select>
import { eliminarPisosApi } from "../../../services/pisoService"; // La nueva función
...
function BorrarPiso() {
    ...
    const handleEdificioChange = (e) => {
        ...
        const edificio = listaEdificios.find(ed => ed.id === parseInt(edificioId));
    ...
    const handleSubmit = async (e) => {
        ...
        // ¡La validación clave que pediste!
        if (cantidad > selectedEdificio.pisos_totales) {
            alert(`No puede borrar ${cantidad} pisos. El edificio "${selectedEdificio.nombre}" solo tiene ${selectedEdificio.pisos_totales} pisos en total.`);
            return;
        }
        
        const isConfirmed = window.confirm(
          `¿Está SEGURO que desea eliminar los últimos ${cantidad} pisos del edificio "${selectedEdificio.nombre}"?\n\nLos pisos ${selectedEdificio.pisos_totales} al ${selectedEdificio.pisos_totales - cantidad + 1} serán borrados.\n\nEsta acción no se puede deshacer.`
        );
    ...
    return (
        ...
        <div className="contenedorAgregar">
            ...
            <p>Seleccione un edificio e ingrese la cantidad de pisos a eliminar (se borrarán los pisos superiores).</p>
            ...
            <form onSubmit={handleSubmit} className="agregar-form">
                ...
                {selectedEdificio && (
                    <>
                        ...
                        <div className="form-group">
                            <label htmlFor="cantidad_borrar">2. Cantidad de pisos a borrar</label>
                            <input 
                                id="cantidad_borrar"
                                type="number"
                                ...
                                placeholder="Ej: 2"
                                max={selectedEdificio.pisos_totales} // Ayuda HTML
                                min="1"
                                disabled={cargando}
                                required
                            />
