...
const ActualizarOficina = () => {
  ...
  // (El resto de tus estados 'useState' y 'useEffect' están correctos)
  ...
  // (Toda tu lógica 'handle...Change' y 'handleSubmit' está correcta)
  ...
  const handleSubmit = async (e) => {
    ...
    const isConfirmed = window.confirm("¿Está seguro que desea actualizar la oficina?");
    if (isConfirmed) {
      try {
        setCargando(true);
        const dataParaActualizar = {
          ...
          persona_id: formData.persona_id === '' ? null : parseInt(formData.persona_id)
        };
        const data = await actualizarOficinaApi(selectedOficina, dataParaActualizar);
  ...
  return (
    <div className="contenedorAgregar">
      ...
      <div className="form-group span-full" style={{ marginBottom: '10px' }}>
        <label>2. Piso:</label>
        <select value={selectedPiso} onChange={handlePisoChange} disabled={!selectedEdificio || cargando}>
      ...
      <div className="form-group span-full" style={{ marginBottom: '20px' }}>
        ...
        <select value={selectedOficina} onChange={handleOficinaChange} disabled={!selectedPiso || cargando}>
