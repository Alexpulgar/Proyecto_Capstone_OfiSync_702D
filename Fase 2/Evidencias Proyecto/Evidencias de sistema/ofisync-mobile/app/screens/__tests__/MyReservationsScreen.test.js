...
describe('MyReservationsScreen', () => {
  ...
  beforeEach(() => {
    ...
    API.put.mockResolvedValue({}); // Mock de complete-past y cancel
  ...
  it('muestra estado de carga', () => {
    ...
    expect(getByText('Cargando mis reservas...')).toBeTruthy();
  });
  ...
  it('muestra mensaje si no hay reservas', async () => {
    ...
    const { findByText } = render(<MyReservationsScreen />);
    expect(await findByText('No tienes reservas activas.')).toBeTruthy();
  });
  ...
  it('llama a la API para completar reservas pasadas y luego lista las reservas del usuario', async () => {
    ...
    expect(await findByText('Sala de Reunión')).toBeTruthy();
    
    // Verifica que se llamó a completar pasadas
    expect(API.put).toHaveBeenCalledWith('/reservations/complete-past');
    // Verifica que se obtuvieron las del usuario
  ...
  it('renderiza la lista de reservas con sus estados', async () => {
    ...
    expect(await findByText('Fecha: 01-11-2025')).toBeTruthy();
  });

  it('muestra el botón "Cancelar" solo para reservas pendientes', async () => {
