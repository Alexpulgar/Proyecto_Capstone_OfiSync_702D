...
jest.mock("@react-native-picker/picker", () => {
  ...
  const MockPicker = (props) => {
    return <mock-Picker {...props}>{props.children}</mock-Picker>;
  };
...
describe("ReserveServiceScreen", () => {
  ...
  describe("Servicio de Impresión (no-room)", () => {
    it("renderiza campos de cantidad, tamaño y archivo", async () => {
      const { getByText, getByPlaceholderText, getByTestId, findByText } =
        render(
          <ReserveServiceScreen
            navigation={mockNavigation}
            route={mockRoutePrint}
          />
        );

      expect(await findByText(mockServicePrint.name)).toBeTruthy();
    ...
    it("valida que la cantidad sea un número positivo", async () => {
      ...
      const cantidadInput = await findByPlaceholderText("Ingrese la cantidad");
      fireEvent.changeText(cantidadInput, "0");
      fireEvent.press(getByText("Reservar"));

      await waitFor(() => {
        expect(Alert.alert).toHaveBeenCalledWith(
          expect.stringContaining("cantidad válida")
        );
      });
    });
    ...
    it("valida que la cantidad no supere 1000", async () => {
      ...
      const cantidadInput = await findByPlaceholderText("Ingrese la cantidad");
      fireEvent.changeText(cantidadInput, "1001");
      fireEvent.press(getByText("Reservar"));
    ...
    it("valida que se seleccione un tamaño", async () => {
      ...
      const cantidadInput = await findByPlaceholderText("Ingrese la cantidad");
      fireEvent.changeText(cantidadInput, "10");
      fireEvent.press(getByText("Reservar"));

      await waitFor(() => {
        expect(Alert.alert).toHaveBeenCalledWith(
          expect.stringContaining("tamaño de hoja")
        );
      });
    });
    ...
    it("valida que se adjunte un archivo", async () => {
      ...
      await findByText(mockServicePrint.name);
      ...
      await waitFor(() => {
        expect(Alert.alert).toHaveBeenCalledWith(
          expect.stringContaining("adjuntar un archivo")
        );
      });
    });
  ...
  describe("Servicio de Sala (room)", () => {
    it("renderiza campos de fecha y hora", async () => {
      ...
      expect(await findByText(mockServiceRoom.name)).toBeTruthy();
    ...
    it("busca horarios ocupados al cargar", async () => {
      ...
      await findByText(mockServiceRoom.name);

      await waitFor(() => {
        expect(API.get).toHaveBeenCalledWith(
          expect.stringContaining("/reservations/room/10/")
        );
      });
    });
  });
