...
export default function ReserveServiceScreen({ route, navigation }) {
  ...
  const pickDocument = async () => {
    try {
      ...
      if (!allowedTypes.includes(selectedFile.mimeType)) {
        Alert.alert("Archivo no válido", "Solo se permiten archivos PDF, DOCX o imágenes (JPG/PNG).");
        return;
      }
  ...
  const fetchBookedSlots = async (selectedDate) => {
    try {
      ...
      const res = await API.get(`/reservations/room/${service.id}/${formattedDate}`);
...
const handleReserve = async () => {
  try {
    ...
    // Enviar reserva
    await fetch(`${API.defaults.baseURL}/reservations`, {
      method: "POST",
      body: formData,
        ...
        <>
          ...
          >
            <Text style={styles.buttonText}>
              {file ? `Archivo: ${file.name}` : "Adjuntar archivo (PDF, DOCX o imagen)"}
            </Text>
          </TouchableOpacity>
        </>
      ) : (
        <>
          ...
          {bookedSlots.length > 0 && (
            <View style={{ marginTop: 10 }}>
              <Text style={{ fontWeight: "bold", marginBottom: 5 }}>Horarios ocupados:</Text>
